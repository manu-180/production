...
function Row({ label, ok, sublabel }: { label: string; ok: "ok" | "warn" | "bad" | "muted"; sublabel?: string }) {
  const tone =
    ok === "ok" ? "bg-emerald-500" :
    ok === "warn" ? "bg-amber-500" :
    ok === "bad" ? "bg-rose-500" : "bg-muted-foreground/40";
  return (
    ...
    <Card>
      <CardContent className="space-y-2 p-4">
        ...
        <Row
          label="Realtime"
          ok={isLive ? "ok" : "warn"}
          sublabel={isLive ? "subscribed" : "connecting…"}
        />
        <Row
          label="Worker"
          ok={isLoading ? "muted" : data?.worker === "ok" ? "ok" : data?.worker === "offline" ? "warn" : "bad"}
        ...
        <Row
          ...
          sublabel={data?.claudeCli.version ?? (data?.claudeCli.installed ? "installed" : "missing")}
        />
      </CardContent>
    </Card>
  );
