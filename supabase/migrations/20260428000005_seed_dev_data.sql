-- Migration: 20260428000005_seed_dev_data.sql
-- Idempotent dev seed. Safe to re-apply. Creates a fixed test user + "Hello Conductor" template.
...
DECLARE
  ...
  v_plan_id uuid;
BEGIN
  -- Test user (fixed UUID for reproducible RLS tests)
  ...
  -- "Hello Conductor" template plan (idempotent: skip if name already exists)
  ...
  WHERE NOT EXISTS (
    ...
    WHERE user_id = v_user_id AND name = 'Hello Conductor'
  )
  RETURNING id INTO v_plan_id;

  -- Prompts (only when the plan was just inserted)
